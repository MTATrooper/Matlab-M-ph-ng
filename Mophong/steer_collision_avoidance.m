function steer = steer_collision_avoidance(Boid, vhl)
  v_pos = Boid(vhl,1:3);
  v_vel = Boid(vhl,4:6);
  v_acc = Boid(vhl,7:9);
  v_maxspeed = Boid(vhl,10);
  v_maxforce = Boid(vhl,11);
  v_max_see_ahead = Boid(vhl,13);
  v_max_avoid_force = Boid(vhl,14);
